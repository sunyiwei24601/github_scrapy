    ...
    for ii in range(1, 100):
        ...
        for rep in repos:
            ...
            if n % 500 == 0:
                ...
                prompt = "The {n}th finding for process {num} is {rate}".format(n=n, num=num, rate=success_rate), time.ctime()
                print(prompt)
                logger.info(prompt)
            ...
            try:
                ...
                name = rep['name']
                url = 'https://api.github.com/repos/{owner}/{repos}/contributors'.format(owner=login, repos=name)
                ...
                try:
                    contributors = list(contributors_tool(url))
                    if len(contributors) >= 1:
                        if len(contributors[0]) >= 1:
                            success_count += 1
                            # print(contributors[0][0])
                
                except requests.exceptions.ConnectionError as e:
                    up = cursor.update_one({'id': idd}, {"$set": {'contributors': [[]]}})
                    continue
                except:
                    traceback.print_exc()
                    ...
                    continue
                #根据id来标记rep并更改
                up = cursor.update_one({'id': idd}, {"$set": {'contributors': contributors}})
            except:
                traceback.print_exc()
                logger.info('There are something wrong for Process{num}'.format(num=num), exc_info=True)
                continue
...
if __name__ == "__main__":
    p = list()
    language = "Ruby"
    p.append(Process(target=get_contributors_process, args=(2160000, 100000, 1, language)))#18150
    p.append(Process(target=get_contributors_process, args=(2260000, 100000, 2, language)))#11500
    p.append(Process(target=get_contributors_process, args=(2360000, 100000, 3, language)))
    p.append(Process(target=get_contributors_process, args=(2460000, 100000, 4, language)))#
    p.append(Process(target=get_contributors_process, args=(2560000, 100000, 5, language)))#
    p.append(Process(target=get_contributors_process, args=(2660000, 100000, 6, language)))#
    p.append(Process(target=get_contributors_process, args=(2760000, 100000, 7, language)))#
    p.append(Process(target=get_contributors_process, args=(2860000, 100000, 8, language)))
